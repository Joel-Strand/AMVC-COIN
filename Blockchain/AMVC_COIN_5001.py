...
class Blockchain:
    ...
    def replace_chain(self):
        network = self.nodes 
        longest_chain = None        # Longest chain is not yet realized.
...
def mine_block():
    ...
    blockchain.add_transaction(sender = node_address,
                               receiver = 'Will',
                               amount = 1)  # Reward for mining block. Not finished.
...
def add_transaction():
    ...
    if not all (key in json for key in transaction_keys):
        return 'Some elements of the transaction or invalid.', 400  # 400 = Bad Request HTTP Code
    index = blockchain.add_transaction(json['sender'], json['receiver', json['amount']])
    response = {f"message': 'Transaction added to Block {index}"}
    return jsonify(response), 201  # 201 = HTTP Successful Post Request
